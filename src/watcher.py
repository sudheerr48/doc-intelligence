"""
File Watcher Module
Handles filesystem events and updates the database in real-time.
"""
...
from .scanner import FileInfo, compute_hash, should_skip
...
class FileChangeHandler(FileSystemEventHandler):
    """
    Watchdog event handler that updates the DuckDB database
    on file create, modify, move, and delete events.
    """
    ...
    def _notify(self, event_type: str, path: str):
        """Call the optional event callback."""
    ...
    def on_moved(self, event: FileSystemEvent):
        if event.is_directory:
            return
        # Remove old path
        self.db.conn.execute("DELETE FROM files WHERE path = ?", [event.src_path])
        # Add new path if it should be processed
