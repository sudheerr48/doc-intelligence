"""
File system health scoring and report generation.
Works standalone (no AI required), with optional AI-enhanced insights.
"""
...
def compute_health_score(metrics: dict) -> dict:
    ...
    if stale_ratio > 0.5:
        ...
        issues.append({
            ...
            "detail": (
                f"{metrics['stale_files']:,} files "
                f"({format_size(metrics['stale_size'])}) "
    ...
    if metrics["duplicate_sets"] > 0:
        summary_parts.append(
            f"{metrics['duplicate_sets']:,} duplicate sets wasting "
            f"{format_size(wasted)}."
        )
...
def generate_health_text(metrics: dict, health: dict) -> str:
    ...
    lines.append(
        f"  Wasted space:      "
        f"{format_size(metrics['wasted_by_duplicates']):>10}"
    )
