"""
Undo / Safety Module
Maintains a manifest of deleted files for recovery within a configurable window.
Every deletion is logged so users can review what was removed and when.
"""
...
def record_deletion(
    ...
    Args:
        original_path: Original file path that was deleted
        size_bytes: Size of the deleted file
        content_hash: Hash of the file contents (for potential recovery)
        reason: Why the file was deleted
        manifest_dir: Custom manifest directory
    """
    ...
    entries.append({
        ...
        "expires_at": (datetime.now() + timedelta(days=DEFAULT_UNDO_WINDOW_DAYS)).isoformat(),
    })
...
def record_batch_deletion(
    ...
    """
    Record multiple file deletions.

    Args:
        files: List of dicts with 'path', 'size_bytes', and optional 'content_hash'
        reason: Why the files were deleted
        manifest_dir: Custom manifest directory
    """
    entries = load_manifest(manifest_dir)
    now = datetime.now()
...
def get_recent_deletions(
    ...
    """
    Get deletions within the undo window.
    ...
    Returns:
        List of deletion records that haven't expired
    """
    entries = load_manifest(manifest_dir)
...
def find_recoverable(
    ...
    """
    Check if a deleted file's content still exists elsewhere (same hash).

    If the same content_hash exists in another file in the database,
    the file can be recovered by copying from that location.

    Args:
        content_hash: Hash of the deleted file
        db: FileDatabase instance
        manifest_dir: Custom manifest directory

    Returns:
        Path to an existing file with the same hash, or None
    """
...
def get_deletion_summary(manifest_dir: Optional[str] = None) -> dict:
    """
    Get a summary of all deletions.

    Returns:
        Dict with total_deleted, total_bytes, recent_count, expired_count
    """
    entries = load_manifest(manifest_dir)
    now = datetime.now()
    ...
    active = [e for e in entries if datetime.fromisoformat(e["expires_at"]) > now]
